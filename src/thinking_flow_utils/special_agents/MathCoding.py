""" 
@file_name: MathCoding.py 
@date: 2023-8-17
@author: Netmind.AI BlackSheep team
This agent can trans the math sentence to the python code, and run the code to get the answer.
"""
...
class MathCoding():
    ...
    def __init__(self, llm="gpt-4-1106-preview", type=1):
        
        self.repeat_time = 0
        
        if type == 1:            
            self.process_1 = BSAgent(template=SYMPT_PROMPT, llm=llm)
        if type == 2:
            self.process_1 = BSAgent(template=SYMPT_PROMPT, llm=llm)
            
        self.debug_agent = BSAgent(template=DEBUG_AGENT, llm=llm)
